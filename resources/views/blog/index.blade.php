      ...
      <section class="container" style="padding: 40px 0 60px;">
        ...
        <div class="posts-grid" style="display:grid; grid-template-columns: repeat(auto-fill, minmax(280px, 1fr)); gap: 20px;">
          @foreach($posts as $post)
            <article class="post-card" style="background: var(--bg-elev); border:1px solid var(--border); border-radius: 16px; overflow:hidden; box-shadow: var(--shadow);">
              ...
              <div style="padding: 14px 16px;">
                ...
                <p style="margin:0 0 12px; color:var(--muted); font-size: 14px;">{{ Str::limit($post->content, 160) }}</p>
                <a class="btn btn-soft" href="{{ route('blog.show', $post->id) }}">Читать →</a>
              </div>
            </article>
          @endforeach
        </div>
